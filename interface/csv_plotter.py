...
class CsvPlotter:
    def __init__(self):
        ...
        self.loaded_csv_label = tk.Label(
            self.root, text="No CSV file loaded", wraplength=380
        )
        ...
        self.y_label.pack(side="left", padx=(5, 0))  # Padding only on the left side
        ...
        self.selected_label = tk.Label(
            self.selected_frame, text="Selected to plot:", anchor="w"
        )
        ...
        self.selected_search_entry.bind("<KeyRelease>", self.search_selected_listbox)
        ...
        self.remove_button = tk.Button(
            self.selected_frame, text="Remove -->", command=self.remove_from_selected
        )
        ...
        self.non_selected_label = tk.Label(
            self.non_selected_frame, text="Non-selected:", anchor="w"
        )
        ...
        self.non_selected_search_entry.bind(
            "<KeyRelease>", self.search_non_selected_listbox
        )
        ...
        self.non_selected_listbox.bind("<Double-Button-1>", self.add_to_selected)
        ...
        self.title_label = tk.Label(self.title_button_frame, text="Plot Title:")
    ...
    def load_csv(self):
        ...
        if file_path:
            try:
                ...
                self.unique_headers = set(col.split("__")[0] for col in self.df.columns)
                self.loaded_csv_label.config(text=f"Loaded CSV file:\n{file_path}")
    ...
    def update_dropdowns(self):
        ...
        self.non_selected_listbox.delete(0, "end")  # Reset non-selected listbox
        for col in x_columns:
            self.x_dropdown_menu["menu"].add_command(
                label=col, command=tk._setit(self.x_dropdown, col)
            )
    ...
    def add_to_selected(self, event=None):
        ...
        for idx in selected_items[
            ::-1
        ]:  # Reverse the selection to maintain order when moving
    ...
    def search_non_selected_listbox(self, event=None):
        self.search_listbox(self.non_selected_search_entry, self.non_selected_listbox)
    ...
    def plot_graph(self):
        ...
        for header_name in self.unique_headers:
            eigen_cols = [
                col for col in selected_cols if col.startswith(header_name + "__")
            ]
    ...
    def plot_graph(self):
        ...
        for header_name in selected_cols:
            y_columns += [
                col for col in self.df.columns if col.startswith(header_name + "__")
            ]
        ...
        if x_column and y_columns:
            ...
            # Create layout
            layout = go.Layout(
                title=self.plot_title_entry.get() if self.plot_title_entry.get() else "Sai2 CSV Plot",
